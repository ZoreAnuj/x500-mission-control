#!/usr/bin/env python3
"""Render a flight CSV to an mp4 that reproduces the live preview (growing trails).
Handles two schemas:
  - replay/infer logs (sp_n/sp_e/sp_d + p_n/p_e/p_d): commanded vs flown
  - teleop logs (p_n/p_e/p_d + vx/vz/yaw_rate): flown only, commands in the title

  python render_replay_video.py runs/replay_ep050.csv --hz 30
  python render_replay_video.py run_01.csv --hz 10
"""
import argparse
...
ap.add_argument("csv")
ap.add_argument("--hz", type=float, default=30.0, help="log rate (for timing)")
ap.add_argument("--stride", type=int, default=2, help="ticks per video frame")
ap.add_argument("--out", default=None)
a = ap.parse_args()

df = pd.read_csv(a.csv)
has_cmd = "sp_n" in df.columns
...
allxy = np.vstack([sp[:, :2], p[:, :2]]) if has_cmd else p[:, :2]
...
allz = np.hstack([-sp[:, 2], -p[:, 2]]) if has_cmd else -p[:, 2]
...
with writer.saving(fig, out, dpi=110):
    for k in range(1, n + 1, a.stride):
        l_p.set_data(p[:k, 1], p[:k, 0])
        dot.set_data([p[k - 1, 1]], [p[k - 1, 0]])
        l_pz.set_data(t[:k], -p[:k, 2])
        if has_cmd:
            l_sp.set_data(sp[:k, 1], sp[:k, 0])
            l_spz.set_data(t[:k], -sp[:k, 2])
            err = float(np.linalg.norm(p[k - 1, :2] - sp[k - 1, :2]))
            info = f"XY err {err:.2f} m"
        else:
            r = df.iloc[k - 1]
            info = " ".join(f"{c}={r[c]:+.2f}" for c in ("vx", "vz", "yaw_rate")
                            if c in df.columns)
        fig.suptitle(f"tick {k - 1}   t={t[k - 1]:.1f}s   {info}")
        writer.grab_frame()
